...
format:
  ...
  pdf: 
    keep-tex: false
    title: Deriving insights for research data sharing from a large-scale data sharing initiative in Canada
    date: last-modified
    ...
    include-in-header:
      text: |
        ...
        \usepackage{fancyhdr}
        \pagestyle{fancy}
        \lhead{\textbf{Research protocol}}
        \rhead{\thepage\ of \pageref*{LastPage}}
        \cfoot{}

---

This document outlines the overall vision, methodological approach, and analytical strategy for the project *Deriving insights for research data sharing from a large-scale data sharing initiative in Canada*. It is intended as a living reference that evolves alongside the project, articulating decisions at the level of design and strategy rather than documenting granular operational steps, which are covered in dedicated sub-protocols. The study's human participants research is conducted under McGill REB file #26-08-029.
...
The primary data source is the email archive of the CITF Databank. A pilot phase established a pipeline to clean, translate, de-identify and de-duplicate an initial set of emails from the inbox of the Databank's central operations manager, producing a small working corpus covering correspondence with 100 partner studies from February 2021 to September 2025 (@fig-email-volume).
...
To check whether the pipeline's output was usable, topic modelling was applied to the processed corpus, identifying 14 coherent topics organized into four thematic clusters. These topics were then analyzed for their distribution across the data sharing lifecycle, across sender roles, and across gender, and for how they co-occurred with one another, surfacing a preliminary set of observations about the structure and timing of coordination work, now reported in the [topic modelling preprint](../topic-modelling/).

The rest of the project's email corpus is assembled by applying the pipeline developed from the pilot phase to additional mailboxes belonging to relevant individuals, including the Databank's Scientific Director and Data Managers. The expansion proceeds iteratively; the data processing and analysis pipeline validated through the pilot is applied to each new mailbox in sequence, with the corpus growing in a controlled and documented fashion. The expanded corpus will then serve as the basis for further analysis, including a reiteration of the topic modelling, a network analysis of communication patterns, and an analysis of the effects of data complexity.

Emails relating to broader CITF operational communications not associated with specific studies are not included. This reflects the project's focus on relationships between the CITF and its partners that contribute data, rather than on other aspects of CITF operations.

### Questionnaire

The secondary data source is a questionnaire administered to individuals who participated in the CITF Databank initiative, identified through the CITF's roster of data contributors, including project leaders, data managers, and key technical and administrative personnel. Project leaders may also be asked to forward the questionnaire to team members who worked in a less visible capacity and whose contact information the CITF does not maintain. A waiver of consent applies to the email data; questionnaire participants provide informed consent prior to beginning. Recruitment and consent procedures are detailed in the ethics protocol, recruitment materials, and informed consent document.

The questionnaire is structured around four domains: (1) participants' professional backgrounds and experiences; (2) independent and collaborative activities; (3) impact and change over time; and (4) demographic data. The first domain also serves as a branching mechanism, routing respondents to role-appropriate question blocks in the remaining sections. Questions in the second and third domains correspond to the topic and phase categories established by the email analysis. The demographic domain collects respondents' gender and institutional affiliation, which correspond with variables structuring the email analysis, therefore enabling comparison across the two data sources at the level of aggregate categories rather than individual identity. The full question set is documented in the questionnaire protocol. A brief pilot with 2–3 respondents will precede full distribution of the questionnaire.

## Methodological Design

The work is carried out in three work packages.

### Work Package 1: Email Corpus Processing
...
![Distribution of multi-topic email assignments across the 14 NMF topics, grouped by thematic category (15% threshold).](visuals/fig_topics_distribution.png){#fig-topics-distribution}

#### Analytical Dimensions

The pilot corpus analysis is reported in the [topic modelling preprint](../topic-modelling/), which covers: topic co-occurrence structure; the distribution of topics across the six phases of the data sharing lifecycle; the distribution of topics and email-to-sender ratios across five sender role categories; and an exploratory gender analysis of coordination labour. As a test of the pipeline on a limited corpus, it produced preliminary observations rather than confirmed findings, including a structural separation between governance and data operations workstreams, an 18:1 ratio of administrative to technical time (median 395 days in DSA negotiation versus 22 days for technical cataloguing), and gendered patterns in topic and phase distributions, all of which the full analysis is designed to test against the expanded corpus.

The expansion of the corpus to additional mailboxes moves the project from a pipeline feasibility test on a single mailbox to the project's actual analysis, extending it across a broader and more organizationally diverse set of perspectives and testing whether the pilot's preliminary observations hold at scale.

<!-- [Make note of the commit hash that distinguishes pre- and post-pilot analysis phases in the topic modelling preprint, once that occurs.] -->
...
A targeted comparison of studies at the extremes of the Intro→DSA timeline distribution (pilot: median 395 days, range 7–1,526) examines their email records for distinguishing organizational patterns. This is supplemented by LLM-assisted qualitative comparison of representative email samples and will inform the identification of studies for targeted questionnaire follow-up, prioritizing those that illustrate theoretically interesting configurations of speed, institutional complexity, and engagement.

#### Impact of Data Complexity

### Work Package 3: Questionnaire Analysis

#### Questionnaire Design

The questionnaire is organized around four sequential domains, detailed in the questionnaire protocol:
...
2. **Independent and collaborative activities.** Addressing specific actions and interactions in the data sharing process, including role-specific working relationships, communication patterns, and moments of friction or facilitation.
3. **Impact and change over time.** Examining participants' experiences with data-sharing initiatives before and after the CITF Databank, and how the experience shaped their subsequent work.
4. **Demographic data.** Collecting gender and institutional affiliation to support aggregate analysis in correspondence with the categories used in the email analysis.

The questionnaire is developed after initial quantitative findings are available, so that findings from the topic modeling, network analysis, and fast/slow study comparison can directly inform its design. Questions in each domain include open-ended items pursuing the same themes in greater depth. Recruitment follows the procedure and materials specified in the ethics protocol and recruitment email, and participants provide consent via the informed consent document prior to beginning the questionnaire.

#### Coding and Analysis

Open-ended questionnaire responses are analyzed using thematic coding. An initial code system is developed from the conceptual framework generated by the email analysis, and iterated as coding proceeds. Analytical memos track cross-cutting themes and contextually rich insights that extend beyond what the code system can represent. Structured (closed-ended) questionnaire responses are analyzed descriptively and associatively, in aggregate categories corresponding to the roles, genders, and institutional affiliations used in the email analysis.

## Planned Outputs

The project is organized around four planned manuscripts.

**Paper 1 — Topic modelling and administrative labour** presents the core topic modelling results. It characterizes the topical landscape of CITF Databank email correspondence, the distribution of topics across the data sharing lifecycle and sender roles, and the gendered patterns of coordination labour. A preprint version of this paper available at [topic-modelling/](../topic-modelling/). We plan to submit the final manuscript to [BMC Medical Informatics and Decision Making](https://link.springer.com/journal/12911).
...
**Sequential integration of methods.** The qualitative component is explicitly designed to follow from the quantitative email analysis. The questionnaire is developed after initial findings are available, making the two work packages intellectually as well as temporally ordered.
...
The questionnaire is subject to limitations inherent in retrospective accounts: participants' reconstructions of past experiences are shaped by subsequent developments and current perspectives. These limitations are partially addressed by grounding questionnaire questions in specific documents and timelines surfaced by the email analysis.
